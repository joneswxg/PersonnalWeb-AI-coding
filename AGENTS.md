...
This is `personal-learning-blog`, a personal learning blog built with Next.js, React, TypeScript, Drizzle ORM, PostgreSQL, NextAuth, and Vitest.

The repository was previously developed with Claude Code. It has been migrated so new agent work should use this file and the `.codex/` directory as the primary guidance surface.
...
- `src/lib/` - Domain logic, query helpers, authorization, markdown, search, and tests.
- `src/db/` - Drizzle database client and schema.
- `drizzle/` - Database migrations and Drizzle metadata.
- `openspec/` - OpenSpec capabilities, proposals, and workflow artifacts.
- `docs/` - Human-readable project documentation.
- `scripts/` - Stable local commands for development, checks, tests, and linting.
- `.codex/` - Codex configuration, rules, hooks, and local skills.
- `.github/` - GitHub automation and Codex prompt assets.

## Common Commands

Use the scripts in `scripts/` as the stable interface for agent work:
...
- Prefer existing patterns in `src/` over introducing new architecture.
- Keep changes scoped to the requested behavior.
- Treat `.env` as local-only and avoid exposing secrets.
- Run focused tests for changed logic, and use `./scripts/check.sh` as the default definition of done.
- OpenSpec remains available for spec-driven work. Use the `.codex/skills/openspec-*` skills when the user asks to propose, explore, apply, sync, or archive a change.
- For OpenSpec work, any completed `opsx:apply` must be followed by a verify step that runs `./scripts/check.sh`. Only run `opsx:archive` after that check passes.
- Git worktrees are supported for feature work. Prefer one branch, one worktree, and one OpenSpec change per feature, and run verification inside the worktree that contains the final code changes.

## Development Workflow

Use `docs/development-workflow.md` as the standard workflow reference for direct changes, OpenSpec changes, verification, archive rules, and git worktree usage.
